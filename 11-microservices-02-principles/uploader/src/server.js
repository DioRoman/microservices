...
app.post("/v1/upload", (request, response) => {
  const bufs = [];
  request.on("data", (d) => {
    console.log(`Received chunk of size: ${d.length}`);
    bufs.push(d);
  });
  request.on("end", async () => {
    const buf = Buffer.concat(bufs);
    console.log(`Total buffer size received: ${buf.length}`);
    ...
    client.putObject(S3_BUCKET, filename, buf, function (err) {
      if (err) {
